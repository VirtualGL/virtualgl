...
class rrprofiler
{
	...
	void endframe(long pixels, long bytes, double incframes)
	{
		...
		{
			hpprintf("%s  ", name);
			if(mpixels) hpprintf("- %.2f Mpixels/sec", mpixels/totaltime);
			if(mbytes) hpprintf("- %.2f Mbits/sec", mbytes*8.0/totaltime);
			if(frames) hpprintf("- %.2f frames/sec", frames/totaltime);
			hpprintf("\n");
