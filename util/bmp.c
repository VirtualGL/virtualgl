	...
	enum BMPPF dstFormat, int flip)
{
	...
	for(j = 0, dstptr = dstbuf; j < height;
		j++, srcptr += flip ? -srcPitch : srcPitch, dstptr += dstPitch)
	{
		for(i = 0, srcptr0 = srcptr, dstptr0 = dstptr; i < width;
			i++, srcptr0 += bmp_ps[srcFormat], dstptr0 += bmp_ps[dstFormat])
		{
	...
	int *height, enum BMPPF format, enum BMPORN orientation)
{
	...
	flags |= O_BINARY;
	#endif
	if(!filename || !buf || !width || !height || format > BMP_NUMPF - 1
		|| align < 1)
	...
	int height, enum BMPPF format, enum BMPORN orientation)
{
	...
	pixelConvert(buf, width, pitch, height, format, tempbuf, width * 3,
		BMPPF_RGB, orientation == BMPORN_BOTTOMUP);
	...
	int height, enum BMPPF format, enum BMPORN orientation)
{
	...
	#endif
	if(!filename || !buf || width < 1 || height < 1 || format > BMP_NUMPF - 1
		|| pitch < 0)
	...
	pixelConvert(buf, width, pitch, height, format, tempbuf, dstPitch, BMPPF_BGR,
