...
void collect(void)
{
	static long long usrJifs[MAXCPUS + 1], niceJifs[MAXCPUS + 1],
		sysJifs[MAXCPUS + 1], totalJifs[MAXCPUS + 1];
	...
	long long usrJif, niceJif, sysJif, idleJif, ioJif, hiqJif, siqJif, totalJif;
	static int first = 1;
	int i, j;
	...
	for(i = 0; i < MAXCPUS + 1; i++)
	{
		...
		if(!first)
		{
			...
			printf("%s: %5.1f (Usr=%5.1f Nice=%5.1f Sys=%5.1f) / Min=%5.1f Max=%5.1f Avg=%5.1f\n",
				i == 0 ? "ALL " : temps2, totalPercent, usrPercent, nicePercent,
				sysPercent, minPercent[i], maxPercent[i],
				grandTotalPercent[i] / (double)iter);
		}
		else
		{
