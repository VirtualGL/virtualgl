...
void findVisual(void)
{
	...
	int pbattribs[] = { GLX_RED_SIZE, 8, GLX_GREEN_SIZE, 8, GLX_BLUE_SIZE, 8,
		...
		None, None };
	int pbattribsdb[] = { GLX_DOUBLEBUFFER, 1, GLX_RED_SIZE, 8,
		...
		if(useFBO)
		{
			...
			if(useRTT)
			{
				...
				glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0,
					useAlpha ? GL_RGBA8 : GL_RGB8, width, height, 0, GL_RGBA,
	...
	unsigned char *buf)
{
	...
	for(j = 0; j < height; j++)
	{
		for(i = 0; i < width; i++)
		{
			...
			{
				buf[(j * width + i) * ps + pf[format].roffset] =
					((j + y) * (i + x)) % 256;
				buf[(j * width + i) * ps + pf[format].goffset] =
					((j + y) * (i + x) * 2) % 256;
				buf[(j * width + i) * ps + pf[format].boffset] =
					((j + y) * (i + x) * 3) % 256;
			}
		}
	}
	...
	int bassackwards)
{
	...
	for(j = 0; j < height; j++)
	{
		...
		for(i = 0; i < width; i++)
		{
			if(pf[format].glFormat == GL_RED)
			{
				if(buf[l * PAD(width * ps) + i * ps] != ((j + y) * (i + x)) % 256)
					return 0;
			}
			else
			{
				if(buf[l * PAD(width * ps) + i * ps + pf[format].roffset]
					!= ((j + y) * (i + x)) % 256)
					return 0;
				if(buf[l * PAD(width * ps) + i * ps + pf[format].goffset]
					!= ((j + y) * (i + x) * 2) % 256)
					return 0;
				if(buf[l * PAD(width * ps) + i * ps + pf[format].boffset]
					!= ((j + y) * (i + x) * 3) % 256)
					return 0;
			}
		}
...
int writeTest(int format)
{
	...
	int n, ps = pf[format].pixelSize, retval = 0;
	double rbtime;
	char temps[STRLEN];
...
int readTest(int format)
{
	...
	try
	{
		...
		if(pbo)
		{
			...
			int temp = 0;
			glGetBufferParameteriv(GL_PIXEL_PACK_BUFFER_EXT, GL_BUFFER_SIZE, &temp);
			if(temp != PAD(width * ps) * height)
		...
		if((rgbaBuffer =
			(unsigned char *)malloc(PAD(width * ps) * height)) == NULL)
			_throw("Could not allocate buffer");
