...
rrframe::rrframe(bool primary) : _bits(NULL), _rbits(NULL), _pitch(0),
...
void rrframe::init(unsigned char *bits, int w, int pitch, int h, int pixelsize,
	int flags)
{
	_bits=bits;
...
rrframe *rrframe::gettile(int x, int y, int w, int h)
{
	rrframe *f;
...
bool rrframe::tileequals(rrframe *last, int x, int y, int w, int h)
{
	bool bu=(_flags&RRFRAME_BOTTOMUP);
		...
		&& _h.dpynum==last->_h.dpynum)
	{
		if(_bits && last->_bits)
		{
			unsigned char *newbits=
				&_bits[_pitch*(bu? _h.height-y-h:y)+_pixelsize*x];
			unsigned char *oldbits=
				&last->_bits[last->_pitch*(bu? _h.height-y-h:y)+_pixelsize*x];
			for(int i=0; i<h; i++)
			{
		...
		{
			unsigned char *newbits=
				&_rbits[_pitch*(bu? _h.height-y-h:y)+_pixelsize*x];
			unsigned char *oldbits=
				&last->_rbits[last->_pitch*(bu? _h.height-y-h:y)+_pixelsize*x];
			for(int i=0; i<h; i++)
			{
				if(memcmp(&newbits[_pitch*i], &oldbits[last->_pitch*i], _pixelsize*w))
					return false;
...
void rrframe::makeanaglyph(rrframe &r, rrframe &g, rrframe &b)
{
	int rindex=_flags&RRFRAME_BGR? 2:0, gindex=1, bindex=_flags&RRFRAME_BGR? 0:2,
		i, j;
	unsigned char *sr=r._bits, *sg=g._bits, *sb=b._bits, *d=_bits, *dr, *dg, *db;

	if(_flags&RRFRAME_ALPHAFIRST) {rindex++;  gindex++;  bindex++;}
...
void rrframe::decompressrgb(rrframe &f, int w, int h, bool righteye)
{
	if(!f._bits || f._h.size<1 || !_bits || !_h.size)
		_throw("Frame not initialized");

	int dstbgr=((_flags&RRFRAME_BGR)!=0), dstbu=((_flags&RRFRAME_BOTTOMUP)!=0),
		dstaf=((_flags&RRFRAME_ALPHAFIRST)!=0);
	int i, j;
...
void rrframe::addlogo(void)
{
	...
	for(int j=0; j<h; j++)
	{
		...
		rowptr+=(_flags&RRFRAME_BOTTOMUP)? -_pitch:_pitch;
	...
	logoptr=vgllogo;
	if(_flags&RRFRAME_BOTTOMUP)
		rowptr=&_rbits[_pitch*(VGLLOGO_HEIGHT+1)
			+(_h.width-VGLLOGO_WIDTH-1)*_pixelsize];
	else
		rowptr=&_rbits[_pitch*(_h.height-VGLLOGO_HEIGHT-1)
			+(_h.width-VGLLOGO_WIDTH-1)*_pixelsize];
...
void rrframe::checkheader(rrframeheader &h)
{
...
rrcompframe::rrcompframe(void) : rrframe(), _tjhnd(NULL)
{
...
rrcompframe::~rrcompframe(void)
{
	if(_tjhnd) tjDestroy(_tjhnd);
}

rrcompframe &rrcompframe::operator= (rrframe &b)
{
	if(!b._bits) _throw("Frame not initialized");
...
void rrcompframe::compressyuv(rrframe& b)
{
	int tjflags=0;
...
void rrcompframe::compressjpeg(rrframe& b)
{
	int tjflags=0;
	...
	tj(tjCompress(_tjhnd, b._bits, b._h.width, b._pitch, b._h.height,
		b._pixelsize, _bits, &size, jpegsub(b._h.subsamp), b._h.qual, tjflags));
...
void rrcompframe::compressrgb(rrframe& b)
{
...
void rrcompframe::init(rrframeheader &h, int buffer)
{
	checkheader(h);
...
void rrfb::init(char *dpystring, Drawable d, Visual *v)
{
	_tjhnd=NULL;
...
void rrfb::init(rrframeheader &h)
{
	...
	_pixelsize=fbx_ps[_fb.format];  _pitch=_fb.pitch;
	_bits=(unsigned char *)_fb.bits;
	_flags=0;
	if(fbx_bgr[_fb.format]) _flags|=RRFRAME_BGR;
	if(fbx_alphafirst[_fb.format]) _flags|=RRFRAME_ALPHAFIRST;
}


rrfb &rrfb::operator= (rrcompframe& f)
{
	int tjflags=0;
	if(!f._bits || f._h.size<1)
		_throw("JPEG not initialized");
	init(f._h);
	if(!_fb.xi) _throw("Frame not initialized");
...
rrxvframe::rrxvframe(char *dpystring, Window win) : rrframe()
{
	init(dpystring, win);
}


void rrxvframe::init(char *dpystring, Window win)
{
...
rrxvframe &rrxvframe::operator= (rrframe &b)
{
	if(!b._bits) _throw("Frame not initialized");
	if(b._pixelsize<3 || b._pixelsize>4)
		_throw("Only true color frames are supported");
...
void rrxvframe::init(rrframeheader &h)
{
	checkheader(h);
