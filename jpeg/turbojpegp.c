...
static const int tjsub[NUMSUBOPT]={SS_111, SS_211, SS_411};
	...
	int jpegsub, int qual, int flags)
{
	...
	PicParm.Get.End    = srcbuf + pitch*(jpegsub==TJ_444? max(height,8):max(height,16));
	...
	PicParm.u.D2S.SubSampling = tjsub[jpegsub];
	...
	if(!(flags&TJ_BGR)) PicParm.u.D2S.PicFlags |= PF_SwapRB;

	PicParm.Put.Start = dstbuf;
	PicParm.Put.End   = dstbuf + TJBUFSIZE(width, height);
	...
	int flags)
{
	...
	if(flags&TJ_ALPHAFIRST) dstbuf=&_dstbuf[1];
	...
	if(!(flags&TJ_BGR)) PicParm.u.S2D.PicFlags |= PF_SwapRB;
