	...
	int jpegsub, int qual, int flags)
{
	...
	j->jdms.free_in_buffer = HPJBUFSIZE(j->cinfo.image_width, j->cinfo.image_height);

	if((row_pointer=(JSAMPROW *)malloc(sizeof(JSAMPROW)*height))==NULL)
		_throw("Memory allocation failed in hpjInitCompress()");
	for(i=0; i<height; i++)
	{
		if(flags&HPJ_BOTTOMUP) row_pointer[i]= &srcbuf[(height-i-1)*pitch];
	...
	*size=HPJBUFSIZE(j->cinfo.image_width, j->cinfo.image_height)-(j->jdms.free_in_buffer);
	...
	int flags)
{
	...
	if((row_pointer=(JSAMPROW *)malloc(sizeof(JSAMPROW)*height))==NULL)
		_throw("Memory allocation failed in hpjInitDecompress()");
	for(i=0; i<height; i++)
	{
		if(flags&HPJ_BOTTOMUP) row_pointer[i]= &dstbuf[(height-i-1)*pitch];
